...
sub viewlist_neighbor :Method {
    ...
    my $carrier  = $carriers[$c->user_carriercode() -1];

    $return_obj   = $memcached->get("$namespace:$carrier:$iArea_areaid");
    ...
    if (!$return_obj) {
        ...
        if (0 <= $return_obj->{LoopNeighborList}) {
            ...
            map {
                    ...
                    $aryref->[$_]->{selfintroduction} = JKZ::Mobile::Emoji::hex2emoji($aryref->[$_]->{selfintroduction});
        ...
        $memcached->replace("$namespace:$carrier:$iArea_areaid", $return_obj, 600);
    }
...
sub regist_area :Method {
 ...
 ## Modified オープンIAREAをやめたので下記を変更 2010/09/21
=pod
  #***************************
  # ドコモ処理
  #***************************
...
## 下記処理は全キャ理統一だけど、取りあえず、ここに停滞 -- 2010/09/21 --
            $lat =~ s/\+//;
            $lon =~ s/\+//;
...
#      ## ドコモエリアコードを出すためにフォーマットを変更
      ...
      ## ドコモエリアコード取得
        ...
        if ( 2 == $c->attrAccessUserData("carriercode") ) {
          ...
          #***************************
            ...
            $lat =~ s/\+//;
            $lon =~ s/\+//;
            $geo  = $q->param('datum');# 0 = WGS-84,  1 = 東京
            #$xacc = $q->param('unit'); # 0 = dms, 1 = degree
        }
