...
# @desc		
#			
# @package	MyClass::JKZDB::BannerImage
# @access	public
# @author	Iwahase Ryo AUTO CREATE BY ./createClassDB
# @create	Tue Jan 12 18:37:17 2010
# @version	1.30
...
$self->{columnslist}->{image}->[$i] = $aryref->[$i]->[3];
$self->{columnslist}->{resized_image}->[$i] = $aryref->[$i]->[4];
...
sub executeUpdate {
	...
	## 設定がない場合はInsert
	#if ($self->{columns}->{bannerdatam_id} < 0) {
...
=pod
		...
		$sqlMoji = sprintf("UPDATE $self->{table} SET %s =?  WHERE bannerdatam_id= '$self->{columns}->{bannerdatam_id}';", join('=?,', @{ $sqlref->[0] }));
		#$rv = $self->executeQuery($sqlMoji, $sqlref->[2]);
