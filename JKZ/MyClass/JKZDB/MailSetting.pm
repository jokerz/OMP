...
# @desc		
#			
# @package	MyClass::JKZDB::MailSetting
# @access	public
# @author	Iwahase Ryo AUTO CREATE BY ./createClassDB
# @create	Fri Jan  8 14:27:54 2010
# @version	1.30
...
$self->{columnslist}->{type}->[$i] = $aryref->[$i]->[1];
$self->{columnslist}->{from_address}->[$i] = $aryref->[$i]->[2];
$self->{columnslist}->{subject}->[$i] = $aryref->[$i]->[3];
$self->{columnslist}->{header}->[$i] = $aryref->[$i]->[4];
$self->{columnslist}->{body}->[$i] = $aryref->[$i]->[5];
$self->{columnslist}->{footer}->[$i] = $aryref->[$i]->[6];
...
## Modified 2009/09/29 BEGIN
		...
		push( @{ $sqlref->[0] }, "type" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{type} ) if $param->{type} ne "";
		push( @{ $sqlref->[0] }, "from_address" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{from_address} ) if $param->{from_address} ne "";
		push( @{ $sqlref->[0] }, "subject" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{subject} ) if $param->{subject} ne "";
		push( @{ $sqlref->[0] }, "header" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{header} ) if $param->{header} ne "";
		push( @{ $sqlref->[0] }, "body" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{body} ) if $param->{body} ne "";
		push( @{ $sqlref->[0] }, "footer" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{footer} ) if $param->{footer} ne "";
