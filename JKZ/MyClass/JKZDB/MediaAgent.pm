...
# @desc		
#			
# @package	MyClass::JKZDB::MediaAgent
# @access	public
# @author	Iwahase Ryo AUTO CREATE BY ./createClassDB
# @create	Fri Feb 19 10:50:25 2010
# @version	1.30
...
$self->{columnslist}->{agent_id}->[$i] = $aryref->[$i]->[0];
$self->{columnslist}->{agent_name}->[$i] = $aryref->[$i]->[1];
$self->{columnslist}->{status_flag}->[$i] = $aryref->[$i]->[2];
$self->{columnslist}->{return_base_url}->[$i] = $aryref->[$i]->[3];
$self->{columnslist}->{kick_back_url}->[$i] = $aryref->[$i]->[4];
$self->{columnslist}->{param_name_for_session_id}->[$i] = $aryref->[$i]->[5];
$self->{columnslist}->{param_name_for_status}->[$i] = $aryref->[$i]->[6];
$self->{columnslist}->{param_name_for_result_session_id}->[$i] = $aryref->[$i]->[7];
$self->{columnslist}->{status_value_for_success}->[$i] = $aryref->[$i]->[8];
$self->{columnslist}->{send_param_name}->[$i] = $aryref->[$i]->[9];
...
#************************ AUTO GENERATED BEGIN ************************
			(
$self->{columns}->{agent_id},
$self->{columns}->{agent_name},
$self->{columns}->{status_flag},
$self->{columns}->{return_base_url},
$self->{columns}->{kick_back_url},
...
sub executeUpdate {
	...
	$self->{columns}->{agent_id} = $param->{agent_id};

	## ここでPrimaryKeyが設定されている場合はUpdate
	## 設定がない場合はInsert
	if ($self->{columns}->{agent_id} < 0) {
...
## Modified 2009/09/29 BEGIN
		...
		push( @{ $sqlref->[0] }, "agent_id" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{agent_id} ) if $param->{agent_id} != "";
		...
		push( @{ $sqlref->[0] }, "return_base_url" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{return_base_url} ) if $param->{return_base_url} ne "";
		push( @{ $sqlref->[0] }, "kick_back_url" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{kick_back_url} ) if $param->{kick_back_url} ne "";
		...
		$sqlMoji = sprintf("UPDATE $self->{table} SET %s =?  WHERE agent_id= '$self->{columns}->{agent_id}';", join('=?,', @{ $sqlref->[0] }));
		#$rv = $self->executeQuery($sqlMoji, $sqlref->[2]);
