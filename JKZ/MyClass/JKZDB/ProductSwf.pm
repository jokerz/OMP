...
# @desc		
#			
# @package	MyClass::JKZDB::ProductSwf
# @access	public
...
$self->{columnslist}->{swf}->[$i] = $aryref->[$i]->[2];
$self->{columnslist}->{mime_type}->[$i] = $aryref->[$i]->[3];
$self->{columnslist}->{file_size}->[$i] = $aryref->[$i]->[4];
$self->{columnslist}->{heigt}->[$i] = $aryref->[$i]->[5];
$self->{columnslist}->{width}->[$i] = $aryref->[$i]->[6];
#************************ AUTO  GENERATED  END ************************
		}
	}
...
sub deleteSwfFileSQL {
	...
	my $rv = $self->{this_dbh}->do($sql, undef, $productm_id, $id);
	return $rv;
}
