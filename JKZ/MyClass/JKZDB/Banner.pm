...
# @desc		
#			
# @package	MyClass::JKZDB::Banner
# @access	public
# @author	Iwahase Ryo AUTO CREATE BY ./createClassDB
# @create	Tue Jan 12 18:36:39 2010
# @version	1.30
...
$self->{columnslist}->{description}->[$i] = $aryref->[$i]->[2];
$self->{columnslist}->{spare1}->[$i] = $aryref->[$i]->[3];
$self->{columnslist}->{spare2}->[$i] = $aryref->[$i]->[4];
...
## Modified 2009/09/29 BEGIN
		...
		push( @{ $sqlref->[0] }, "spare1" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{spare1} ) if $param->{spare1} != "";
		push( @{ $sqlref->[0] }, "spare2" ), push( @{ $sqlref->[1] }, "?" ), push( @{ $sqlref->[2] }, $param->{spare2} ) if $param->{spare2} != "";
