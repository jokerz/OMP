...
# @author    Iwahase Ryo
# @create    2009/01/21
# @update    2009/03/12    __session_id __login_name __password メソッド追加
# @update    2009/03/31    __level メソッド追加
# @update    2009/07/27    AM getSubCategoryOBJ
# @update    2009/08/27    serverinfoメソッドをJKZApp::AppBaseSettingに移動
# @version   1.02
#******************************************************
package MyClass::JKZApp;
...
#__PACKAGE__->mk_accessors( qw( cfg query dbh ) );
...
sub dispatch {
    ...
    ## Modified 2009/03/31
    ($self->{session_id}, $self->{login_name}, $self->{password}, $self->{name}, $self->{level}) = split(/::/, $cookie);
    $self->setMicrotime("t0");
...
$sql = "
SELECT
 DATE_FORMAT(NOW(), '%Y年%m月%d日') AS TODAY ,
 COUNT(IF(orderstatus_flag=1, ordernum, 0)) AS MIKAKUNIN,
 COUNT(IF(orderstatus_flag=2, ordernum, 0)) AS KAKUNIN
 FROM tOrderDataF
 WHERE DATE(order_date)=DATE(NOW());
";
( $obj->{TODAY}, $obj->{TODAY_ORDERCOUNT_NEW}, $obj->{TODAY_ORDERCOUNT} ) = $dbh->selectrow_array($sql);

## 先日の商品注文数
    $sql = "
SELECT
 DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 1 DAY), '%Y年%m月%d日') AS YESTERDAY,
 COUNT(IF(orderstatus_flag=1, ordernum, 0)) AS MIKAKUNIN,
 COUNT(IF(orderstatus_flag=2, ordernum, 0)) AS KAKUNIN
 FROM tOrderDataF
 WHERE DATE(order_date)=DATE(DATE_SUB(NOW(),INTERVAL 1 DAY));
";
( $obj->{YESTERDAY}, $obj->{YESTERDAY_ORDERCOUNT_NEW}, $obj->{YESTERDAY_ORDERCOUNT}) = $dbh->selectrow_array($sql);
...
$sql = "
SELECT
 DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 1 MONTH), '%Y年%m月') AS LASTMONTH,
 COUNT(IF(status_flag=2 AND carrier=2, owid, NULL)) AS RD,
 COUNT(IF(status_flag=2 AND carrier=4, owid, NULL)) AS RSB,
 COUNT(IF(status_flag=2 AND carrier=8, owid, NULL)) AS RA,
 COUNT(IF(status_flag=2, owid, NULL)) AS TR,
 COUNT(IF(status_flag=4 AND carrier=2, owid, NULL)) AS WD,
 COUNT(IF(status_flag=4 AND carrier=4, owid, NULL)) AS WSB,
 COUNT(IF(status_flag=4 AND carrier=8, owid, NULL)) AS WA
 FROM tMemberM WHERE MONTH(registration_date)=MONTH(DATE_SUB(NOW(), INTERVAL 1 MONTH))
;";
=cut

=pod
## Modified 今月の会員入退会者数 退会合計も追加 2010/02/26
## 登録ログテーブルからの集計に変更
$sql = "
SELECT
 DATE_FORMAT(NOW(), '%Y年%m月') AS THISMONTH,
 COUNT(IF(status_flag=2 AND carrier=2, owid, NULL)) AS RD,
 COUNT(IF(status_flag=2 AND carrier=4, owid, NULL)) AS RSB,
 COUNT(IF(status_flag=2 AND carrier=8, owid, NULL)) AS RA,
 COUNT(IF(status_flag=2, owid, NULL)) AS TR,
 ...
 COUNT(IF(status_flag=4 AND carrier=4, owid, NULL)) AS WSB,
 COUNT(IF(status_flag=4 AND carrier=8, owid, NULL)) AS WA,
 COUNT(IF(status_flag=4, owid, NULL)) AS TW
 FROM tUserRegistLogF WHERE MONTH(date_of_transaction)=MONTH(NOW())
;";
...
## ランキングオブジェクトファイル
my $userpointranking_objectfile = sprintf("%s/%s.obj", $self->cfg->param('USERPOINT_RANK_DIR'), $yyyymm);
...
my $aryref;
    ...
    $obj->{LoopRankingList} = $#{ $aryref };
    for (my $i = 0; $i <= $obj->{LoopRankingList}; $i++) {
        $obj->{owid}->[$i]      = $aryref->[$i]->[0];
        $obj->{nickname}->[$i]  = $aryref->[$i]->[1];
        $obj->{SUM_POINT}->[$i] = $aryref->[$i]->[2];
        $obj->{my_rank}->[$i]   = $i+1;
    }
$obj->{ERROR_MSG} = $userpointranking_objectfile;

    $obj->{DUMP} = Dumper($aryref);
=cut

my $bbb;# = $self->getFromObjectFile({CONFIGURATION_VALUE=>$userpointranking_objectfile});

    eval {
        $bbb = MyClass::WebUtil::publishObj( { file=>$userpointranking_objectfile } );
    };
...
my $abc = $bbb->[0];
        ...
        $obj->{SUM_POINT} = $abc->[2];
        $obj->{my_rank}   = "1";
        $obj->{LoopADList} = $#{ $abc->[3] };
        for (my $j=0; $j<=$obj->{LoopADList}; $j++) {
            $obj->{ad_id}->[$j] = $abc->[3]->[$j]->[0];
            $obj->{ad_name}->[$j] = $abc->[3]->[$j]->[1];
         }
$obj->{ERROR_MSG} = $userpointranking_objectfile;
$obj->{DUMP} = Dumper($bbb);
    return $obj;
...
sub logout {
    ...
    if (defined ($session_id)) {
        ...
        #my $sess_ref = JKZ::Session->open(undef, $session_id);
        $sess_ref->delete();
        $sess_ref->close();
        $self->deletecookie();
...
sub _processCMSHtml {
   ...
   ## Modified 2011/11/07 JKZHtmlクラスの仕様変更に伴いクッキーはハッシュで指定
    #$myHtml->doPrintTags((!$self->{cookie} ? undef : $self->{cookie}));
    $myHtml->doPrintTags((!$self->{cookie} ? undef : { cookie => $self->{cookie} }));
}
...
sub memberlist {
    ...
    my $myMemberList = MyClass::JKZDB::Member->new($dbh);
    my %condition = (
        whereSQL   => 'status_flag = 2',
        orderbystr =>    'mid ASC',
    );
    $myMemberList->executeSelectList(\%condition);

    $myMemberList->{LoopMemberList} = $myMemberList->countRec();

    $self->_processHtml($myMemberList);
}
...
sub fetchTmplt {
    ...
    my $obj       = $memcached->get("1MPTmpltList");
    if (!$obj) {
        my $dbh         = $self->getDBConnection();
        ...
        ## 30分有効キャッシュ
        $memcached->add("1MPTmpltList", $obj, 1800);
        ## 最大のIDを取得してキャッシュに入れる
        $obj->{max_tmplt_id} = $myTmpltList->maxTmpltID() + 1;
        $memcached->add("1MPTmpltMaxID", $obj->{max_tmplt_id}, 1800);
...
sub rebuildCache {
    ...
    if ($q->param('f')) {
        ...
        my @modules = $self->CONFIGURATION_VALUE("GENERATE_SMALL_SUB_CATEGORY_LIST", "GENERATE_RECOMMEND_PRODUCT", "GENERATE_LATEST_PRODUCT", "GENERATE_NEWS", "FLUSH_MEMCAHED_MODULE");
