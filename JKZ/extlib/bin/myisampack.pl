...
=pod
    $myisampack = `/usr/bin/find / -type f -name 'myisampack'`;
    $myisamchk  = `/usr/bin/find / -type f -name 'myisamchk'`;
    $mysqladmin = `/usr/bin/find / -type f -name 'mysqladmin'`;
=cut
...
=pod
    ...
    foreach my $job (0..$cnt_find_things) {
        sleep 1;
        $mysqltools[$job] = $find_things->[$job];
        $prog->update($job);
    }
=cut