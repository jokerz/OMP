...
my $outputdir= '/home/vhosts/OMP/JKZ/modules/mail/regist/';
...
sub main {
    ...
    #####@使用テーブルのhash
    my %TABLE = (
                    'mailsetting' => 'OMP.tMailSettingM',
                    'member'      => 'OMP.tMemberM',
                    'maillog'     => 'OMP.tMailAddressLogF',
                );
    ...
    $sql = sprintf("SELECT session_id FROM %s WHERE guid=? AND new_mobilemailaddress=?;", $TABLE{maillog});
    ...
    $msg = sprintf("下記のURLにアクセスして登録を完了させてください。
                    http://omp.1mp.jp/rg.mpl?t=1&s=%s",
                $session_id);
...
RETURN_MAIL_EXIT:
    ...
    $sql = sprintf("SELECT from_address, subject, header, body, footer FROM OMP.tMailSettingM WHERE id=?;", $TABLE{mailsetting});
